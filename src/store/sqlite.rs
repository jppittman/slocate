...
impl Db for SqliteDb {
    fn begin(&self) -> crate::error::Result<()> {
    ...
    fn insert_chunk(&self, c: &Chunk, v: &[f32]) -> crate::error::Result<()> {
        ...
        self.conn.execute(
            "INSERT OR REPLACE INTO chunks_fts (rowid, name, source) VALUES (?1, ?2, ?3)",
    ...
    fn get_chunks_by_ids(&self, ids: &[String]) -> crate::error::Result<Vec<Chunk>> {
        ...
        let p: String = ids
            ...
            .join(",");
        let sql = format!(
            "SELECT id, kind, name, source_path, source FROM chunks WHERE id IN ({})",
            p
        );
        let mut s = self.conn.prepare(&sql)?;
        let r = s.query_map(rusqlite::params_from_iter(ids), |row| {
            Ok(Chunk {
                id: row.get(0)?,
                kind: ChunkKind::from_db_str(&row.get::<_, String>(1)?),
                name: row.get(2)?,
    ...
    fn get_chunks_with_vectors_by_ids(
        &self,
        ids: &[String],
    ) -> crate::error::Result<Vec<(Chunk, Vec<f32>)>> {
        if ids.is_empty() {
            return Ok(Vec::new());
        }
        let p: String = ids
            ...
            .join(",");
        let sql = format!(
            "SELECT id, kind, name, source_path, source, vector FROM chunks WHERE id IN ({})",
            p
        );
        let mut s = self.conn.prepare(&sql)?;
        let r = s.query_map(rusqlite::params_from_iter(ids), |row| {
    ...
    fn update_file_meta(&self, entries: &[(String, FileMeta)]) -> crate::error::Result<()> {
        let mut s = self.conn.prepare_cached(
            "INSERT OR REPLACE INTO file_meta (rel_path, mtime_ns, size) VALUES (?1, ?2, ?3)",
        )?;
        for (p, m) in entries {
            s.execute(params![p, m.mtime_ns, m.size])?;
        }
        Ok(())
    }
    ...
    fn remove_files(&mut self, rel_paths: &[String]) -> crate::error::Result<()> {
        ...
        for path in rel_paths {
            ...
            let mut s =
                tx.prepare("SELECT rowid, name, source FROM chunks WHERE source_path = ?1")?;
            let chunks: Vec<(i64, String, String)> = s
    ...
    fn remove_chunks_for_file(&self, rel_path: &str) -> crate::error::Result<()> {
        let mut s = self
            .conn
            .prepare("SELECT rowid, name, source FROM chunks WHERE source_path = ?1")?;
        let chunks: Vec<(i64, String, String)> = s
    ...
    fn bm25_search(&self, query: &str, limit: usize) -> crate::error::Result<Vec<(String, f32)>> {
        ...
        let b = r[0].1;
        Ok(r.into_iter()
            .map(|(id, s)| (id, if b == 0.0 { 0.0 } else { (s / b) as f32 }))
            .collect())
    }
...
mod tests {
    ...
    fn test_bm25_or_logic() -> crate::error::Result<()> {
        ...
        assert_eq!(
            results.len(),
            1,
            "Expected 1 result for 'log2 missing_word', got {:?}",
            results
        );
        ...
        assert_eq!(
            results.len(),
            1,
            "Expected 1 result for 'log2 neon', got {:?}",
            results
        );
        assert_eq!(results[0].0, "1");
